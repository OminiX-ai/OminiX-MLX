...
impl QwenFeedForward {
    pub fn new(dim: i32) -> Result<Self, Exception> {
        ...
        Ok(Self {
            mlp_in: QuantizedLinearBuilder::new(dim, hidden_dim).build()?,
            mlp_out: QuantizedLinearBuilder::new(hidden_dim, dim).build()?,
        })
    }
...
impl QwenAttention {
    pub fn new(dim: i32, num_heads: i32, head_dim: i32) -> Result<Self, Exception> {
        Ok(Self {
            ...
            num_heads,
            head_dim,
            to_q: QuantizedLinearBuilder::new(dim, dim).build()?,
            to_k: QuantizedLinearBuilder::new(dim, dim).build()?,
            to_v: QuantizedLinearBuilder::new(dim, dim).build()?,
            add_q_proj: QuantizedLinearBuilder::new(dim, dim).build()?,
            add_k_proj: QuantizedLinearBuilder::new(dim, dim).build()?,
            add_v_proj: QuantizedLinearBuilder::new(dim, dim).build()?,
            ...
            attn_to_out: vec![QuantizedLinearBuilder::new(dim, dim).build()?],
            to_add_out: QuantizedLinearBuilder::new(dim, dim).build()?,
        })
    }
    ...
    pub fn forward(
        ...
        if !DEBUG_BEFORE_NORM.swap(true, std::sync::atomic::Ordering::SeqCst) {
        ...
        if !DEBUG_NORM.swap(true, std::sync::atomic::Ordering::SeqCst) {
            mlx_rs::transforms::eval([&img_q, &txt_q]).ok();
            let img_q_norm = mlx_rs::ops::sqrt(&mlx_rs::ops::sum_axis(&mlx_rs::ops::multiply(&img_q, &img_q).unwrap(), -1, false).unwrap()).unwrap();
            let txt_q_norm = mlx_rs::ops::sqrt(&mlx_rs::ops::sum_axis(&mlx_rs::ops::multiply(&txt_q, &txt_q).unwrap(), -1, false).unwrap()).unwrap();
        ...
        if !DEBUG_QK.swap(true, std::sync::atomic::Ordering::SeqCst) {
        ...
        // encoder_hidden_states_mask: [B, txt_seq] with 1 for real tokens, 0 for padding
        // Create joint mask: [text_mask, 1s_for_image] and convert to additive form
        if let Some(mask) = encoder_hidden_states_mask {
            let img_seq = img_modulated.dim(1);
            let ones_img = Array::ones::<f32>(&[batch, img_seq])?;
            // Joint mask: [B, txt_seq + img_seq]
            ...
            let additive_mask = ops::multiply(&ops::subtract(&one, &joint_mask)?, &neg_inf)?;
            // Reshape for broadcasting: [B, 1, 1, joint_seq]
            let additive_mask = additive_mask.reshape(&[batch, 1, 1, txt_seq + img_seq])?;
            attn_scores = ops::add(&attn_scores, &additive_mask)?;

            // Debug mask on first call
            static DEBUG_MASK: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);
            if !DEBUG_MASK.swap(true, std::sync::atomic::Ordering::SeqCst) {
                let mask_sum = mask.sum(None).unwrap();
                mlx_rs::transforms::eval([&mask_sum]).ok();
                eprintln!("[DEBUG MASK] Text mask sum (num real tokens): {:.0}", mask_sum.item::<f32>());
            }
        }

        // Debug: check raw attention scores before softmax
        static DEBUG_SCORES: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);
        if !DEBUG_SCORES.swap(true, std::sync::atomic::Ordering::SeqCst) {
            mlx_rs::transforms::eval([&attn_scores]).ok();
            // Check scores for first image token attending to text vs image
            let scores_slice = attn_scores.index((0, 0, txt_seq, ..));  // First image query's scores
            mlx_rs::transforms::eval([&scores_slice]).ok();
            eprintln!("[DEBUG SCORES] Raw attention scores (before softmax):");
            eprintln!("  To text positions: min={:.4}, max={:.4}, mean={:.4}",
                scores_slice.index((..txt_seq,)).min(None).unwrap().item::<f32>(),
                scores_slice.index((..txt_seq,)).max(None).unwrap().item::<f32>(),
                scores_slice.index((..txt_seq,)).mean(None).unwrap().item::<f32>());
            eprintln!("  To img positions: min={:.4}, max={:.4}, mean={:.4}",
                scores_slice.index((txt_seq..,)).min(None).unwrap().item::<f32>(),
                scores_slice.index((txt_seq..,)).max(None).unwrap().item::<f32>(),
                scores_slice.index((txt_seq..,)).mean(None).unwrap().item::<f32>());
        }
        ...
        // Debug: check attention pattern on first call (use static flag)
        static DEBUG_ONCE: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);
        if !DEBUG_ONCE.swap(true, std::sync::atomic::Ordering::SeqCst) {
            mlx_rs::transforms::eval([&attn]).ok();
            eprintln!("[DEBUG ATTN] scores shape: {:?}", attn.shape());
            // Check if image tokens attend to text tokens
            // attn shape: [B, heads, joint_seq, joint_seq] where joint_seq = txt_seq + img_seq
            let attn_slice = attn.index((0, 0, txt_seq, ..));  // First image query's attention
            mlx_rs::transforms::eval([&attn_slice]).ok();
            eprintln!("[DEBUG ATTN] First img token attention:");
            eprintln!("  To text positions: {:.6}", attn_slice.index((..txt_seq,)).mean(None).unwrap().item::<f32>());
            eprintln!("  To img positions: {:.6}", attn_slice.index((txt_seq..,)).mean(None).unwrap().item::<f32>());
            // Check total attention to text vs image
            let attn_to_txt = attn.index((0, .., txt_seq.., ..txt_seq)).mean(None).unwrap().item::<f32>();
            let attn_to_img = attn.index((0, .., txt_seq.., txt_seq..)).mean(None).unwrap().item::<f32>();
            eprintln!("  Avg img->txt: {:.6}, Avg img->img: {:.6}", attn_to_txt, attn_to_img);
            // Verify: sum should be close to 1 per query row
            let sum_txt = attn_to_txt * txt_seq as f32;
            let sum_img = attn_to_img * (attn.dim(3) - txt_seq) as f32;
            eprintln!("  Sum check: txt={:.4}, img={:.4}, total={:.4}", sum_txt, sum_img, sum_txt + sum_img);
        }
...
impl QwenTransformerBlock {
    pub fn new(dim: i32, num_heads: i32, head_dim: i32) -> Result<Self, Exception> {
        Ok(Self {
            dim,
            img_mod_linear: QuantizedLinearBuilder::new(dim, dim * 6).build()?,
            txt_mod_linear: QuantizedLinearBuilder::new(dim, dim * 6).build()?,
            attn: QwenAttention::new(dim, num_heads, head_dim)?,
            img_ff: QwenFeedForward::new(dim)?,
            txt_ff: QwenFeedForward::new(dim)?,
        })
    }
    ...
    pub fn forward(
        ...
        if !DEBUG_TEMB.swap(true, std::sync::atomic::Ordering::SeqCst) {
        ...
        if !DEBUG_MOD.swap(true, std::sync::atomic::Ordering::SeqCst) {
            mlx_rs::transforms::eval([&img_mod_params]).ok();
            eprintln!("[DEBUG IMG_MOD_PARAMS] shape: {:?}, min={:.4}, max={:.4}, mean={:.4}",
                img_mod_params.shape(),
                img_mod_params.min(None).unwrap().item::<f32>(),
                img_mod_params.max(None).unwrap().item::<f32>(),
                img_mod_params.mean(None).unwrap().item::<f32>());
        }
        ...
        if !DEBUG_GATE.swap(true, std::sync::atomic::Ordering::SeqCst) {
...
impl QwenTimestepEmbedder {
    pub fn new(timestep_dim: i32, inner_dim: i32) -> Result<Self, Exception> {
        Ok(Self {
            linear_1: QuantizedLinearBuilder::new(timestep_dim, inner_dim).build()?,
            linear_2: QuantizedLinearBuilder::new(inner_dim, inner_dim).build()?,
        })
    }
    ...
    pub fn forward(&mut self, t: &Array) -> Result<Array, Exception> {
        ...
        if !DEBUG_T.swap(true, std::sync::atomic::Ordering::SeqCst) {
...
impl QwenTimeTextEmbed {
    pub fn new(timestep_dim: i32, inner_dim: i32) -> Result<Self, Exception> {
        Ok(Self {
            timestep_embedder: QwenTimestepEmbedder::new(timestep_dim, inner_dim)?,
        })
    }
...
impl QwenAdaLayerNormOut {
    pub fn new(inner_dim: i32) -> Result<Self, Exception> {
        Ok(Self {
            linear: QuantizedLinearBuilder::new(inner_dim, inner_dim * 2).build()?,
        })
    }
...
impl QwenQuantizedTransformer {
    pub fn new(config: QwenConfig) -> Result<Self, Exception> {
        ...
        Ok(Self {
            img_in: QuantizedLinearBuilder::new(config.in_channels, inner_dim).build()?,
            txt_norm: QwenTransformerRMSNorm::new(config.joint_attention_dim)?,
            txt_in: QuantizedLinearBuilder::new(config.joint_attention_dim, inner_dim).build()?,
            time_text_embed: QwenTimeTextEmbed::new(256, inner_dim)?,
            transformer_blocks,
            norm_out: QwenAdaLayerNormOut::new(inner_dim)?,
            proj_out: QuantizedLinearBuilder::new(inner_dim, output_dim).build()?,
            config,
        })
    }
    ...
    pub fn forward(
        ...
        if !DEBUG_TXT_RAW.swap(true, std::sync::atomic::Ordering::SeqCst) {
        ...
        if !DEBUG_TXT_NORMED.swap(true, std::sync::atomic::Ordering::SeqCst) {
            mlx_rs::transforms::eval([&encoder_hidden_states]).ok();
        ...
        if !DEBUG_TXT.swap(true, std::sync::atomic::Ordering::SeqCst) {
            mlx_rs::transforms::eval([&encoder_hidden_states]).ok();
...
fn layer_norm(x: &Array, eps: f32) -> Result<Array, Exception> {
    ...
    let std = ops::sqrt(&ops::add(&variance, &eps_arr)?)?;
    ops::divide(&x_centered, &std)
}
